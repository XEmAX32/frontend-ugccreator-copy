...
const AVAILABLE_VOICES = [
  { id: "aria", name: "Aria", voiceId: "9BWtsMINqrJLrRacOk9x", gender: "female", accent: "american", isFavorite: false, category: "professional" },
  { id: "roger", name: "Roger", voiceId: "CwhRBWXzGAHq8TQ4Fs17", gender: "male", accent: "british", isFavorite: true, category: "professional" },
  { id: "sarah", name: "Sarah", voiceId: "EXAVITQu4vr4xnSDxMaL", gender: "female", accent: "american", isFavorite: false, category: "casual" },
  { id: "laura", name: "Laura", voiceId: "FGY2WhTYpPnrIDTdsKH5", gender: "female", accent: "american", isFavorite: true, category: "casual" },
  { id: "charlie", name: "Charlie", voiceId: "IKne3meq5aSn9XLyUdCD", gender: "male", accent: "american", isFavorite: false, category: "casual" },
  { id: "george", name: "George", voiceId: "JBFqnCBsd6RMkjVDRZzb", gender: "male", accent: "british", isFavorite: false, category: "professional" },
  { id: "callum", name: "Callum", voiceId: "N2lVS1w4EtoT3dr4eOWO", gender: "male", accent: "british", isFavorite: false, category: "casual" },
  { id: "river", name: "River", voiceId: "SAz9YHcvj6GT2YYXdXww", gender: "nonbinary", accent: "american", isFavorite: false, category: "casual" },
  { id: "liam", name: "Liam", voiceId: "TX3LPaxmHKxFdv7VOQHJ", gender: "male", accent: "american", isFavorite: false, category: "professional" },
  { id: "charlotte", name: "Charlotte", voiceId: "XB0fDUnXU5powFXDhCwa", gender: "female", accent: "british", isFavorite: false, category: "professional" },
];
...
const VoiceSelection = () => {
  ...
  const filteredVoices = voices.filter((voice) => {
    ...
    const matchesTab = 
      (activeTab === "all") || 
      (activeTab === "favorites" && voice.isFavorite);
  ...
  const toggleFavorite = (voiceId: string, event: React.MouseEvent) => {
    ...
    setVoices(voices.map(voice => {
      if (voice.id === voiceId) {
        ...
        return { ...voice, isFavorite: newStatus };
      }
      return voice;
    }));
  };
      ...
      <div className="container px-4 py-6 relative bg-transparent min-h-screen">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col space-y-4 mb-6">
            ...
            <ToggleGroup type="single" value={activeCategory} onValueChange={(value) => setActiveCategory(value || "all")} className="flex flex-wrap gap-2">
              <ToggleGroupItem value="all" className="bg-black/20 text-white border-white/10 data-[state=on]:bg-white data-[state=on]:text-black">
                All
              </ToggleGroupItem>
              <ToggleGroupItem value="professional" className="bg-black/20 text-white border-white/10 data-[state=on]:bg-white data-[state=on]:text-black">
                Professional
              </ToggleGroupItem>
              <ToggleGroupItem value="casual" className="bg-black/20 text-white border-white/10 data-[state=on]:bg-white data-[state=on]:text-black">
                Casual
              </ToggleGroupItem>
            </ToggleGroup>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-6 max-h-[65vh] overflow-y-auto no-scrollbar pb-12 scroll-smooth">
            {filteredVoices.length === 0 ? (
              ...
              filteredVoices.map((voice) => (
                <Card 
                  key={voice.id}
                  className={`relative overflow-hidden cursor-pointer transition-all duration-200 hover:transform hover:scale-[1.03] border-0 bg-black/20 ${
                  ...
                  <div className="p-4 relative">
                    <div className="flex justify-between items-start">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        className={`text-white/80 hover:text-white ${voice.isFavorite ? 'text-yellow-400 hover:text-yellow-300' : ''}`}
                    ...
                    <div className="flex items-center justify-between mt-4">
                      ...
                      <Button
                        ...
                        onClick={(e) => showVoiceDetails(voice, e)}
                      >
                        More Info
                      </Button>
                    </div>
            ...
            <div className="mt-4 space-y-4">
              <div className="flex justify-center">
                <Button
                  variant="outline"
                  className="bg-gray-700/50 border-gray-600/50 text-white"
                  onClick={(e) => playVoiceSample(selectedVoiceDetails.id, e)}
                >
